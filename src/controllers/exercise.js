...
async function getAll(req, res, next){
  try{
    ...
    next({status:400, error: `Unable to get exercise logs`})
  }
}
...
async function getId(req, res, next){
  try{
    ...
    res.status(200).json({"exercise_log": response})
  } catch (e){
    next({status:400, error: `Unable to find specified exercise log`})
  }
}
...
async function createLog(req, res, next){
  try{
    ...
    res.status(201).json({"exercise_log": response})
  } catch (e){
